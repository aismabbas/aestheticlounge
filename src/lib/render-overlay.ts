...
async function loadFonts() {
  if (!fontsPromise) {
    fontsPromise = (async () => {
      const base = process.env.NEXT_PUBLIC_SITE_URL || process.env.URL || 'https://aesthetic-lounge-dev.netlify.app';
      const [playfairReg, playfairBold, inter] = await Promise.all([
        fetch(`${base}/fonts/PlayfairDisplay-Regular.ttf`).then(r => r.arrayBuffer()),
        fetch(`${base}/fonts/PlayfairDisplay-Bold.ttf`).then(r => r.arrayBuffer()),
